...
{   
 ...
 //QMessageBox::information(this,tr("treeBeenClicked"),tr("device"));
        ...
        (*it).setZeroChanNum(struDeviceInfoV40.struDeviceV30.byZeroChanNum);

            char secretKey[16] = {0};
            strncpy(secretKey, (*it).getEncryptionKey().toLatin1().data(), 16);
            NET_DVR_SetSDKSecretKey(i, secretKey);
...
// QMessageBox::information(this,tr("treeBeenClicked"),tr("channel"));
        ...
        {   
            ...
            char secretKey[16] = {0};
            strncpy(secretKey, (*it).getEncryptionKey().toLatin1().data(), 16);
            NET_DVR_SetSDKSecretKey(ret, secretKey);
...
{   
    ...
    if (m_iposttreelevel == 0)
    {
		...
		m_addnewdevice = new AddNode;
 		...
 		connect(m_addnewdevice->ui.cancel_pushButton,SIGNAL(clicked()),this, SLOT(addDeviceCancelClicked()));
 		m_addnewdevice->exec();
...
QList<DeviceData> QtClientDemo::getListFromArray(QByteArray& data)
{
    ...
    for ( QStringList::iterator it = lines.begin(); it != lines.end(); ++it)
    {
        ...
        {   
            ...
            str = *(it+6);
            newDevice->setEncryptionKey(str);
        }
...
void QtClientDemo::loginDevice()
{
    ...
    else
    {
        ...
        char secretKey[16] = {0};
        strncpy(secretKey, (*it).getEncryptionKey().toLatin1().data(), 16);
        NET_DVR_SetSDKSecretKey((*it).m_iuserid, secretKey);
...
{	
    ...
    newDNode->setEncryptionKey(m_addnewdevice->ui.encryptionkey_lineEdit->text());
    ...
    {
        char secretKey[16] = {0};
        strncpy(secretKey, newDNode->getEncryptionKey().toLatin1().data(), 16);
        NET_DVR_SetSDKSecretKey(newDNode->m_iuserid, secretKey);
    }
    ...
    {   
		...
		m_addnewdevice->ui.passwd_lineEdit->setText(newDNode->m_qpassword);
 	    ...
 	    connect(m_addnewdevice->ui.cancel_pushButton,SIGNAL(clicked()),this,
...
// The following IP channel loop has been disabled to prevent duplicate camera entries.
