                   ...
                   LogTable * table, QWidget *parent): QWidget(parent)
{
	...
	// Channel Zero state
	m_rpzerochannelflag = 0;
	m_rpzerochannelhandle = -1;
	m_rpzerochanneldevicerow = -1;
    ...
    connect(ui.pushButton_zerochannel, SIGNAL(clicked()), this, SLOT(on_pushButton_zerochannel_clicked()));
}
...
RealPlay::~RealPlay()
{
    ...
    // Clean up frame windows to avoid lingering X resources
    for (int i = 0; i < REALPLAY_MAX_NUM; ++i) {
        if (m_framePlayWnd[i] != NULL) {
            delete m_framePlayWnd[i];
            m_framePlayWnd[i] = NULL;
        }
    }
    ...
    if (m_rpzerochannelflag == 1)
    {
        stopZeroChannelRealPlay();
    }
...
void RealPlay::RealPlayFullScreenRep()
{
    ...
    ui.pushButton_zerochannel->hide();
...
void RealPlay::RealPlayNormalFrameRep()
{
   ...
   // gqtclinetdemo->ui.toolBar_2->show();
    ...
    ui.pushButton_zerochannel->show();
...
NET_DVR_CLIENTINFO tmpclientinfo;
                if (m_framePlayWnd[i] == NULL) { continue; }
                HWND winId = (HWND)m_framePlayWnd[i]->GetPlayWndId();
                if (winId == 0 || !m_framePlayWnd[i]->isVisible()) { continue; }
                tmpclientinfo.hPlayWnd = winId;
...
NET_DVR_CLIENTINFO tmpclientinfo;
	...
	{
		NET_DVR_CLIENTINFO tmpclientinfo;
        HWND winId = (HWND)m_pSelectedPlayWnd->GetPlayWndId();
        if (winId == 0) { QMessageBox::warning(this, tr("Play error"), tr("Selected play window is unavailable.")); return; }
        tmpclientinfo.hPlayWnd = winId;
...
/*******************************************************************
      Function:   RealPlay::on_pushButton_zerochannel_clicked
   Description:   Start or stop Channel Zero stream
     Parameter:   none
        Return:   none
**********************************************************************/
void RealPlay::on_pushButton_zerochannel_clicked()
{
    if (*m_rpuserid < 0)
    {
        QMessageBox::information(this, tr("have no login yet"), tr("not login"));
    ...
    if (m_rpzerochannelflag == 0)
    {
        startZeroChannelRealPlay();
    }
    else
    {
        stopZeroChannelRealPlay();
        m_rpzerochannelflag = 0;
        ui.pushButton_zerochannel->setText(tr("Channel Zero"));
    }
}

/*******************************************************************
      Function:   RealPlay::startZeroChannelRealPlay
   Description:   Start Channel Zero stream on the selected device
     Parameter:   none
        Return:   none
**********************************************************************/
void RealPlay::startZeroChannelRealPlay()
{
    int devicerow = -1;
    ...
    if (devicerow < 0)
    {
        QMessageBox::information(this, tr("Channel Zero"), tr("Please select a device first."));
        return;
    }

    int ret = startZeroChannelRealPlayEncapseInterface(devicerow);
    if (ret == 1)
    {
        m_rpzerochannelflag = 1;
        m_rpzerochanneldevicerow = devicerow;
        ui.pushButton_zerochannel->setText(tr("Stop Zero"));
        ui.comboBox_windowsnum->setCurrentIndex(0);
        comboWinNumChanged(0);
    }
}

/*******************************************************************
      Function:   RealPlay::stopZeroChannelRealPlay
   Description:   Stop Channel Zero stream
     Parameter:   none
        Return:   none
**********************************************************************/
void RealPlay::stopZeroChannelRealPlay()
{
    if (m_rpzerochannelhandle >= 0)
    {
        NET_DVR_ZeroStopPlay(m_rpzerochannelhandle);
        m_rpzerochannelhandle = -1;
    }

    if (m_rpzerochanneldevicerow >= 0)
    {
        stopZeroChannelRealPlayEncapseInterface(m_rpzerochanneldevicerow);
    }
...
/*******************************************************************
      Function:   RealPlay::startZeroChannelRealPlayEncapseInterface
   Description:   Start Channel Zero via NET_DVR_ZeroStartPlay
     Parameter:   int devicerow - device index in m_qlistdevicetree
        Return:   1 on success, 0 on failure
**********************************************************************/
int RealPlay::startZeroChannelRealPlayEncapseInterface(int devicerow)
{
    QList<DeviceData>::iterator it;
    int i = 0;
    for (it = (*m_qlistdevicetree).begin(); it != (*m_qlistdevicetree).end(); ++it)
    ...
    if (!(*it).isZeroChanSupported())
    {
        QMessageBox::information(this, tr("Channel Zero"),
            tr("This device does not support Channel Zero (byZeroChanNum = 0).\n"
               "Please enable Channel Zero in the device web interface first."));
        return 0;
    }
    ...
    struClientInfo.hPlayWnd = (HWND)m_framePlayWnd[0]->GetPlayWndId();

    LONG realhandle = NET_DVR_ZeroStartPlay((*it).getUsrID(), &struClientInfo, RealDataCallBack, NULL, TRUE);

    if (realhandle < 0)
    {
        DWORD dwError = NET_DVR_GetLastError();
        QString errMsg;
        if (dwError == NET_ERR_NO_ZERO_CHAN)
        {
            errMsg = tr("Channel Zero not available on this device (Error 953: NET_ERR_NO_ZERO_CHAN).\n"
                        "Please enable Channel Zero in the device web interface.");
        }
        else
        {
            errMsg = tr("NET_DVR_ZeroStartPlay error, SDK_LASTERROR=%1").arg(dwError);
        }
        QMessageBox::information(this, tr("Channel Zero Error"), errMsg);
        return 0;
    }

    m_rpzerochannelhandle = realhandle;
    m_rpcurrentrealhandle = realhandle;
    m_rpuseridbackup = (*it).getUsrID();
...
/*******************************************************************
      Function:   RealPlay::stopZeroChannelRealPlayEncapseInterface
   Description:   Stop Channel Zero stream and update device state
     Parameter:   int devicerow - device index in m_qlistdevicetree
        Return:   none
***********************************************************************/
void RealPlay::stopZeroChannelRealPlayEncapseInterface(int devicerow)
{
    QList<DeviceData>::iterator it;
    int i = 0;
    for (it = (*m_qlistdevicetree).begin(); it != (*m_qlistdevicetree).end(); ++it)
